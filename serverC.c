...
int main(void){
	...
	if((rv = getaddrinfo(LOCAL_HOST, SERVER_C_UDP_PORT, &hints, &servinfo)) != 0){
	...
	for (p = servinfo; p != NULL; p = p->ai_next){
		if((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1){
			perror(SERVER_C_SOCKET_ERROR_MSG);
			continue;
		}
		if(bind(sockfd, p->ai_addr, p->ai_addrlen) == -1){
			close(sockfd);
			perror(SERVER_C_BIND_ERROR_MSG);
			continue;
		}
		break;
	}
	if(p == NULL){
		fprintf(stderr, "%s\n", SERVER_C_BIND_ERROR_MSG);
		return 0;
	}
	freeaddrinfo(servinfo);
	printf("The Server %s is up and running using UDP on port %s.\n", SERVER_C_NAME, SERVER_C_UDP_PORT);
